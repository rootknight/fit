...
type OctokitCallMethods = {
  ...
  createTreeNodeFromFile: (
    { path, status, extension }: LocalChange,
    remoteTree: TreeNode[]
...
export class Fit implements IFit {
  ...
  async computeLocalSha(): Promise<{ [k: string]: string }> {
    const paths = this.vaultOps.vault
      ...
      .filter(Boolean);
    // 添加配置文件路径
    paths.push(this.vaultOps.vault.configDir);
  ...
  async remoteUpdated(): Promise<{
    ...
    return {
      remoteCommitSha,
      updated: remoteCommitSha !== this.lastFetchedCommitSha,
    };
  }

  async getLocalChanges(
    currentLocalSha?: Record<string, string>
  ): Promise<LocalChange[]> {
    if (!currentLocalSha) {
  ...
  async getLatestRemoteCommitSha(
  ...
  async getRemoteTreeSha(tree_sha: string): Promise<{ [k: string]: string }> {
    ...
    const remoteSha = Object.fromEntries(
      remoteTree
        .map((node: TreeNode): [string, string] | null => {
